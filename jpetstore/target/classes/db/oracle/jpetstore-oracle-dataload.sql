...
SELECT 'p2p' || lpad(post_seq.nextval,3,0) FROM DUAL;
SELECT * 
...
SELECT itemnum.NEXTVAL FROM DUAL;

SELECT *
FROM user_sequences WHERE sequence_owner = 'SSD01_4';

SELECT LAST_NUMBER FROM USER_SEQUENCES WHERE SEQUENCE_NAME = 'TB_ZZTRACE_SQ01';
DROP SEQUENCE post_seq;

SELECT ITEMNUM.NEXTVAL 
     FROM dual;
     
select /*+ index_desc(tablename seqindexname) */ seq from product where rownum = 1;




grant create sequence to ssd01_4;

INSERT INTO SSD01_4.PRODUCT (PRODUCTID)
      VALUES
      (POST_SEQ.NEXTVAL);
...
create sequence productnum increment by 1 START WITH 100 nocache;
delete from sequence where name = 'productnum';
select productnum.nextval from dual;
SELECT *
FROM USER_SEQUENCES;
...
grant select,alter on root.itemnum to ssd01_4 ;
...
select
	...
	FROM SSD01_4.P2P P
	WHERE ITEMID='EST-6';
	ALTER TABLE P2P ADD constraint FK_P2P_1 FOREIGN KEY (ITEMID) REFERENCES ITEM(ITEMID) ON DELETE CASCADE;
